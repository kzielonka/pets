...
import SignIn, { Api } from '../SignIn.vue'
import type { SignUpApi, SetAccessToken } from '../ApiProvider.vue'
...
describe('SignIn', () => {
  ...
  const callSignIn: SignUpApi = (email: string, password: string) => {
  ...
  it('sends email and password to server and sets access token recieved in response', async () => {
    ...
    const callSignIn: SignUpApi = vi.fn((email: string, password: string) => Promise.resolve({ success: true, accessToken }));
    const setAccessToken: SetAccessToken = vi.fn((accessToken: string) => {});
    const api: Api = { callSignIn, setAccessToken  };
